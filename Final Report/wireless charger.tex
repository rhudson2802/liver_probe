...
%Cut last bit with parasitic resistances?
...
%Cut out hpwm explanation This allows PWM to be set up simply by specifying the output pin, frequency and duty cycle \cite{picbasic_pro}.
...
%Cut out explanation of TMR2 increments
...
The calculated energies (see table \ref{tab: remote energy}) were then combined in a weighted sum to determine the number of readings which could be taken per full battery charge. When the probe was not in use, the button would activate every \SI{100}{\milli\second} and then the system would enter a delay period. This resulted in a stand-by time of 9.88 hours. If the probe is in constant use, so every button poll results in a measurement being taken, then 48,500 measurements can be taken, which corresponds to a lifetime of 8.56 hours. As the probe will be left on trickle charge when it is not in use, these lifetimes will be suitable for the probe to retain its charge during the transplant operation.\\
